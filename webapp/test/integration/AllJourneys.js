jQuery.sap.require("sap.ui.qunit.qunit-css");
jQuery.sap.require("sap.ui.thirdparty.qunit");
jQuery.sap.require("sap.ui.qunit.qunit-junit");
QUnit.config.autostart = false;

// We cannot provide stable mock data out of the template.
// If you introduce mock data, by adding .json files in your webapp/localService/mockdata folder you have to provide the following minimum data:
// * At least 3 BusinessPartnerCollection in the list
// * All 3 BusinessPartnerCollection have at least one RelatedContacts

sap.ui.require([
	"sap/ui/test/Opa5",
	"com/test/teched/test/integration/pages/Common",
	"sap/ui/test/opaQunit",
	"com/test/teched/test/integration/pages/App",
	"com/test/teched/test/integration/pages/Browser",
	"com/test/teched/test/integration/pages/Master",
	"com/test/teched/test/integration/pages/Detail",
	"com/test/teched/test/integration/pages/NotFound"
], function (Opa5, Common) {
	"use strict";
	Opa5.extendConfig({
		arrangements: new Common(),
		viewNamespace: "com.test.teched.view."
	});

	sap.ui.require([
		"com/test/teched/test/integration/MasterJourney",
		"com/test/teched/test/integration/NavigationJourney",
		"com/test/teched/test/integration/NotFoundJourney",
		"com/test/teched/test/integration/BusyJourney"
	], function () {
		QUnit.start();
	});
});